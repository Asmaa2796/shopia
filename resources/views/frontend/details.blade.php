...
@section('content')
    ...
    <div class="products py-4">
        <div class="container">
            <div>
                <a href={{ url('/') }}>Home</a> \ <span>{{ $product->category->name }}</span> \
            ...
            <div class="card bg-white rounded border shadow-sm my-2 p-4 product_data">
                <div class="row details">
                    ...
                    <div class="col-lg-7 col-md-7 col-12">
                        ...
                        <hr>
                        <b class="d-inline-block mb-3">Product ratings:</b>
                        @php
                        ...
                        @endfor
                        <small class="text-secondary text-sm">({{ number_format($ratings->count()) }} Ratings)</small>
                        <div>
                            <b class="d-block">Price:</b>
                            <del class="text-danger text-sm">{{ number_format($product->original_price) }} EGP</del>
                            <b class="text-success">{{ number_format($product->selling_price) }} EGP</b>
                        </div>
                        @if ($product->qty > 0)
                            <div class="badge bg-success text-white my-2">In stock</div>
                            <b class="d-block mt-2">Quantity</b>
                            ...
                            <div class="my-3">
                                <a href="{{ url('add_to_cart') }}" class="custom-btn1 add_to_cart">Add
                                    to cart <i class="fas fa-shopping-cart fa-sm"></i></a>
                                <a href="{{ url('add_to_wishlist') }}"
                                    class="custom-btn2 add_to_wishlist">Add to Wishlist <i
                                        class="fas fa-heart fa-sm"></i></a>
                            </div>
                        @else
                    ...
                    <div class="col-lg-6 col-md-6 col-12">
                        ...
                        <div class="px-2 py-1 border d-inline-block rounded mb-3">
                            <button type="button" data-toggle="modal" data-target="#rating" class="btn pl-0 btn-link text-warning"><i
                                class="fas fa-star fa-sm"></i> Rate this product</button>
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <a href="{{ url('add_review/' . $product->slug . '/user_review') }}"
                                ...
                                Add review <i class="fas fa-comment"></i>
                            </a>
                        </div>
                        <hr>
                        @if ($reviews->count() > 0)
                                ...
                                <div class="user_review">
                                    ...
                                    @if ($review->user_id == Auth::id())
                                        <a href="{{ url('edit_review/' . $product->slug . '/user_review') }}"
                                            class="btn-sm text-secondary btn-link"><i class="fas fa-pen fa-sm"></i> Edit</a>
                                    @endif
                                    <br>
                                    <small>
                                        @php
        ...
        <div class="container">
            ...
            @if ($related_products->count() > 0)
                                ...
                                <div class="content">
                                    ...
                                    <div>
                                        <a class="shadow-sm view_details"
                                            href="{{ url('details/' . $item->category->slug . '/' . $item->slug) }}">View
                                            details</a>
                ...
                <div class="bg-white shadow-sm p-5 border rounded my-5 text-center">
                    <h4 class="my-0">No Products exists</h4>
                </div>
            @endif
        </div>
    </div>
